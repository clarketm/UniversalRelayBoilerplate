...
export default class UrlRouter {
  ...
  static traverseRouteRecursive(route) {
    ...
    if (children) {
      if (children.constructor === Array)
        for (let childRoute of route.props.children) UrlRouter.traverseRouteRecursive(childRoute)
      else UrlRouter.traverseRouteRecursive(children)
    }
  }

  static addRouteToTree(branch, url, arrUrl, ixArrUrl) {
    // If we are past the end of the URL, place the entire url/route name
    if (arrUrl.length == ixArrUrl) branch['/'] = url
    else {
      ...
      if (urlSegment.startsWith(':')) {
        ...
        if (nextParameter != null)
          throw new Error(
            'There can be only one unique url segment parameter when adding ' +
              parameterName +
              ', got ' +
        ...
        if (nextBranch == null) nextBranch = branch[urlSegment] = {}
  ...
  static goToRouteByNameAndParams(routeName, params) {
    ...
    for (ix = currentlyOpenRoutes.length - 1; ix >= 0; ix--)
      if (currentlyOpenRoutes[ix] == routeName) {
        ...
        if (popNum > 0) {
          for (var i = 0; i < popNum; i++) currentlyOpenRoutes.pop()
  ...
  static goToRouteByURLRecursive(branch, arrUrl, ixArrUrl, params) {
    if (arrUrl.length == ixArrUrl) UrlRouter.goToRouteByNameAndParams(branch['/'], params)
    else {
      const urlSegment = arrUrl[ixArrUrl]
      let nextBranch = branch[urlSegment]
