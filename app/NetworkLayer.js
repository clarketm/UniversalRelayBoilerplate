...
export default class NetworkLayer {
    ...
    Keychain.getGenericPassword()
      .then(credentials => {
        ...
        NetworkLayer.setUserTokens(
          credentialsJSON.UserToken1,
          credentialsJSON.UserToken2,
          false,
          cb,
        )
      })
  ...
  static setUserTokens(UserToken1, UserToken2, persist, cb) {
    ...
    if (UserToken1 != null) headers.Cookie = 'UserToken1=' + UserToken1

    if (UserToken2 != null) headers.UserToken2 = UserToken2

    const graphQLServerURL = publicURL + '/graphql'

    // Create network layer with options and inject
    currentEnvironment.injectNetworkLayer(
            ...
            return next(req).then(res => {
              if (res.json.error) {
                ...
                if (res.json.error == 'Authentication Failed') {
                  console.log(
                    'TODO: x2000 Somehow alert the user. Your account could not be found. You have been logged out.',
                  )
    ...
    if (listeningComponent) listeningComponent.updateEnvironment(UserToken1 == null)
